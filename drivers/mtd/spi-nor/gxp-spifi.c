...
static int gxp_spi_prep(struct spi_nor *nor, enum spi_nor_ops ops)
{
	struct gxp_spifi_drvdata *spifi = nor->priv;
...
static void gxp_spi_unprep(struct spi_nor *nor, enum spi_nor_ops ops)
{
	struct gxp_spifi_drvdata *spifi = nor->priv;
...
static int gxp_spi_read_reg(struct spi_nor *nor, u8 opcode, u8 *buf, int len)
{
	int ret = 0;
...
static int gxp_spi_write_reg(struct spi_nor *nor, u8 opcode, u8 *buf, int len)
{
	int ret = 0;
	struct gxp_spifi_drvdata *spifi = nor->priv;
	void __iomem *reg_base = spifi->reg_base;
				...
				struct device_node *np)
{
	...
	nor->prepare = gxp_spi_prep;
	nor->unprepare = gxp_spi_unprep;
	nor->read_reg = gxp_spi_read_reg;
	nor->write_reg = gxp_spi_write_reg;
	nor->read = gxp_spi_read;
	nor->write = gxp_spi_write;
	nor->erase = gxp_spi_erase;
	nor->priv  = spifi;
